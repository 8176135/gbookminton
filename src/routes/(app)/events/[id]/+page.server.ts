...
export const load = (async ({ locals, params }) => {
    ...
    const listed = signups.filter((s) => s.signup.status === 'listed' || s.signup.status === 'locked');
    ...
    if (!isAdmin && ev.isPrivate) {
        return {
            ...
            signups: null,
            userSignupStatus: signups.find((s) => s.signup.userId === session.user.id)?.signup.status ?? null
        };
    }
...
export const actions: Actions = {
    ...
    editEvent: async ({ request, locals, params }) => {
        ...
        if (!title || !dateStr || !deadlineStr || isNaN(duration) || isNaN(capacity) || isNaN(costDollars)) {
